...
interface NavTab { label: string; icon: string; route: string; }

const NAV_TABS: NavTab[] = [
  { label: 'Home',    icon: 'home',            route: '/'             },
  { label: 'Chat',    icon: 'smart_toy',        route: '/chat'         },
  { label: 'Planner', icon: 'calendar_month',   route: '/meal-planner' },
  { label: 'Pantry',  icon: 'kitchen',          route: '/pantry'       },
...
@Component({
  ...
  template: `
    <div class="app-shell">
        ...
        @if (auth.isLoggedIn()) {
          <div class="px-3 py-3 d-flex align-items-center gap-2 border-bottom">
            ...
            <div class="overflow-hidden flex-fill">
              <div class="fw-semibold text-truncate" style="font-size:13px">{{ auth.currentUser()?.full_name || 'Welcome' }}</div>
              <div class="text-muted text-truncate" style="font-size:11px">{{ auth.currentUser()?.email }}</div>
            </div>
      ...
      <!-- ══════════════════════════════════════════
           MOBILE: top navbar
      ══════════════════════════════════════════ -->
      <nav class="navbar d-flex d-md-none px-2 bg-white border-bottom shadow-sm top-0 position-sticky" style="z-index:100;height:56px">
        <button class="btn btn-sm btn-light rounded-circle p-2" (click)="menuOpen.set(!menuOpen())" aria-label="Menu">
        ...
        <div class="d-flex align-items-center gap-1 fw-bold" style="color:#2e7d32;font-size:15px">
          <mat-icon>eco</mat-icon> Organic Care
        </div>
        <button class="btn btn-sm btn-light rounded-circle p-2 notif-bell-btn" aria-label="Notifications"
      ...
      @if (menuOpen()) {
        ...
        <nav class="side-drawer d-flex flex-column fade-in">
          <div class="d-flex align-items-center gap-2 px-4 py-4 border-bottom fw-bold" style="color:#2e7d32;font-size:17px">
            <mat-icon>eco</mat-icon> Organic Care AI
          </div>
          @if (auth.isLoggedIn()) {
          ...
          <div class="border-top mt-auto">
            @if (auth.isLoggedIn()) {
              ...
              <a class="drawer-link d-flex align-items-center gap-3 px-4 py-3 text-decoration-none"
                 routerLink="/auth/login" (click)="menuOpen.set(false)">
                <mat-icon style="color:#6b7c6b">login</mat-icon> Sign In
              </a>
            }
          </div>
        </nav>
      ...
      <!-- Floating chat widget — hidden on /chat, /notifications and /auth/* pages -->
  ...
  styles: [`
    ...
    .sidebar-link {
      ...
      &:hover { background: #f2f5f0; color: #2e7d32; mat-icon { color: #2e7d32; } }
      &.active {
    ...
    .sidebar-bell-btn {
      ...
      border: none; background: #f2f5f0; color: #6b7c6b;
      display: flex; align-items: center; justify-content: center;
    ...
    .drawer-link {
      font-size: 15px; font-weight: 500; color: #1a2a1a;
      transition: background 0.15s; cursor: pointer;
      &:hover { background: #f2f5f0; }
    }
